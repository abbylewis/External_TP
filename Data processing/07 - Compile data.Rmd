...
surface_avgs <- read.csv("../Compiled data/summer_averages_wi_surface.csv")%>%
  dplyr::select(LakeID,Layer,Year,Chla_ugL,Chla_date,
                Temp_C,Temp_date,DOC_mgL,TP_ugL,TN_ugL,
                TP_date,TN_date)

summer_avgs_wide <- summer_avgs%>%
  dplyr::select(-Chla_ugL,-Chla_date,-DOC_mgL)%>%
  full_join(strat_avgs)%>%
  full_join(surface_avgs)%>%
...
with_temp <- with_temp_all%>%
  ...
  filter(!LakeID=="123",#No clear seasons
