...
us_map_epi_chla <- map_trend(epi_tp_sen %>% filter(!is.na(trend)), "Epi. chl-a (µg/L/y)")
...
all_trends <- tp_input_trend_conc %>%
  ...
  full_join(epi_tp_sen %>% mutate(var = "TP_ugL_EPI")) %>%
  full_join(hypo_tp_sen %>% mutate(var = "TP_ugL_HYPO")) %>%
...
Plot correlation between in-lake and input concentrations (i.e., not trends)
```{r}
...
On average no evidence for an effect of any month of TP input on WI lake concentrations
```{r}
