...
to_profiles <- to_profiles_all%>%
  ...
  filter(is.na(MaximumDepth_m)|MaximumDepth_m > 1)%>%
...
nrow(to_profiles %>% filter(reported_max_depth > 1)) #The loss of profiles is not primarily because of shallow lakes
...
## export
write.csv(database.interp, "../Compiled data/temp_o2_interpolated_unstratified.csv")

#number of profiles before and after
#oxygen
to_profiles %>% filter(!is.na(DO_mgL)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() 
database.interp %>% filter(!is.na(DO_mgL)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() 
#temp
to_profiles %>% filter(!is.na(Temp_C)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() 
database.interp %>% filter(!is.na(Temp_C)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() 
